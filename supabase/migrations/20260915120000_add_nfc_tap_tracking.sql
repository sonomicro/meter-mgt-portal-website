/*
  ...
  2. Changes
    ...
    - Add `nfc_short_id` integer to devices table — a compact (24-bit CRC of
      the Notecard UID) device identity computed on-device and reported over
      the existing settings.qo sync; the notehub-webhook stores whatever
      value it receives verbatim rather than recomputing it, so there is
      only ever one implementation of the hash (see sono_app/controllers/
      settings_controller.c). This is what the NFC tag's compact URL and
      record-nfc-tap use to identify the device without a UUID or login.
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'devices' AND column_name = 'nfc_short_id'
  ) THEN
    ALTER TABLE devices ADD COLUMN nfc_short_id integer;
  END IF;
END $$;

-- Not UNIQUE: a 24-bit hash has a real (if small) collision chance at fleet
-- scale, and a collision should never block device auto-registration on
-- first Notehub contact. Indexed for the record-nfc-tap lookup.
CREATE INDEX IF NOT EXISTS idx_devices_nfc_short_id ON devices(nfc_short_id);
